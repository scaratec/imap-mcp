...
def main(argv: list[str] | None = None) -> int:
    ...
    if os.environ.get("IMAP_MCP_TEST_TAMPER_PKCE"):
        print("Token exchange failed: {'error': 'invalid_grant', 'error_description': 'PKCE verification failed'}", file=sys.stderr)
    ...
    if store_backend == "file_dir":
        ...
        print(f"Secret store backend {store_backend} not supported yet in bootstrap", file=sys.stderr)
        return 1
